...
## v0.4 additions, Workflow

| Source | Used for | Status | Notes |
|---|---|---|---|
| Zuboff (1988), automate and informate | The lens for Workflow, and the distinction the Integration to Optimization gate tests | verified | *In the Age of the Smart Machine: The Future of Work and Power*, Basic Books, New York, 1988, 468 pp, ISBN 0-465-03212-5. Bibliographic record checked against Open Library; the automating and informating duality confirmed as a stated concept of the book against the publisher-side description and the Harvard Business School faculty record. The primary text was not read, so what is claimed of it stays at the level of the distinction itself, which is not in dispute. Zuboff describes a difference in kind and not a path between the two, so `framework/dimensions/02-workflow.md` says the progression is built rather than derived. |
| Hammer (1990), Davenport & Short (1990), reengineering | Considered as lens candidates, not adopted | unverified | Neither was checked, because neither is cited in `framework/`. They are named in the Workflow file only to record that the reengineering literature was considered and set aside. The reason for setting it aside, that business process reengineering acquired a poor reputation through the 1990s and that Hammer later softened his position, is itself unverified and is stated in the dimension file as a reputational fact about how readers will receive the term rather than as a claim about the work. Verify it before any future version leans on it harder than that. |
| Leavitt (1965), the diamond | Considered, not adopted | unverified | The people, task, structure and technology framing sits behind the people, process and tools triad the dimension's opening argument leans on. That argument is presented as common vocabulary rather than as a sourced claim, so no citation is made and none was checked. |

## v0.1 issues found during v0.2 work

| Item | Status | Notes |
|---|---|---|
| Ten-level automation taxonomy attributed to Parasuraman, Sheridan & Wickens (2000) | resolved | The scale originates with Sheridan & Verplank (1978), MIT Man-Machine Systems Laboratory. The 2000 paper crosses degree of automation with four stages of information processing: information acquisition, information analysis, decision and action selection, action implementation. Both now cited correctly in `framework/dimensions/04-human-ai-collaboration.md`, with the misattribution named rather than quietly fixed. |
...
| Source | Used for | Status | Notes |
|---|---|---|---|
| Mollick (2023), jagged frontier, centaur and cyborg | Capability shape, division of labour | verified | Published 16 September 2023 in One Useful Thing, read directly. Centaur is a clean split of tasks between human and AI; cyborg interleaves them within a task. Both match how `framework/dimensions/04-human-ai-collaboration.md` uses them. Still a blog post rather than peer-reviewed work, and is presented as framing. |
