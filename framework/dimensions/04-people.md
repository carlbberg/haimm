...
*Failure signal.* A well-attended kickoff meeting with no record of a single concern raised. Silence read as buy-in, which Satir's model calls late status quo holding, not agreement.
...
*Failure signal.* A pilot that shipped on schedule with resistance noted in a slide and no visible response to it. Resistance acknowledged and resistance addressed are different, and only one shows up in outcomes.
...
*Failure signal.* One or two people who are visibly excellent at the new workflow and everyone else routing work through them. Individual mastery reads as team integration on a satisfaction survey and is not.
...
*Failure signal.* A single well-attended training from eighteen months ago, cited whenever the topic comes up. One event is not a culture, and continuous evolution is exactly the distinction a single training cannot demonstrate.
