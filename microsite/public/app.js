...
function profileVerdict(a) {
  if (a.some(stage => stage === null)) return 'Some dimensions are still unplaced. Treat those unknowns as useful questions and gather one recent example before deciding.';
  if (new Set(a).size === 1) return 'Your selections form an even profile. Check the evidence behind that consistency before treating every placement as established.';
  return 'Your team is developing at different speeds. That uneven shape is the diagnosis, and it helps you choose the next conversation by consequence.';
}
...
function renderResults() {
  ...
  }).join('')}</div><div class="result-close"><span>You have a starting point. Choose one useful conversation.</span><button id="restart" class="text-button">Retake the assessment</button></div>`;
