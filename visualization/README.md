...
**It is stale now.** The working tree rewrote twenty matrix cells across Solution Fit, Human-AI Collaboration, People and Ethics, added a governance criterion to Human-AI Collaboration's Integration to Optimization gate, and retagged one Knowledge & Context criterion from [S] to [O]. None of that is in the file below. It was left alone deliberately: several of the new cells are longer than the ones they replace, the artboard is a fixed 1440 by 900 whose cell type was already dropped to 9.8px to fit six rows, and a hand-edit that clips is worse than an honest note. Rebuild it with the canvas tooling before anybody presents from it.
